...
namespace NinetiesTV
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Print("80's Genres", EightiesGenres(shows));
            Print("All Genres", GetGeners(shows));
        ...
        **************************************************************************************************/
        ...
        {
            return ShowsByPopularity(shows).Take(3).ToList();
        }
        ...
        {
            return shows.OrderByDescending(s => s.StartYear).Where(s => s.Genres.Count > 1).ToList();
        }
        ...
        **************************************************************************************************/
        ...
        {
            return shows.Where(s => s.StartYear < 1990 && s.StartYear > 1979).SelectMany(s => s.Genres).Distinct().ToList();
        }
        // 2. Print a unique list of geners.
        static List<string> GetGeners(List<Show> shows)
        {
        ...
        // static string ShowsByYear(List<Show> shows)
        // {
        //     shows = shows.OrderBy(p => p.StartYear).ToList();
        //     string showList = "";
        //     int numShowsByYear = 0;
        //     var showsByYear = shows.GroupBy(s => s.StartYear).ToList();
        //     foreach (var q in showsByYear)
        //     {
        //         showList += q.Key;
        //         foreach (Show show in q)
        //         {
        //             numShowsByYear++;
        //         }
